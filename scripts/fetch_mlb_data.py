...
async def fetch_team_data(team: TeamConfig, executor, semaphore):
    async with semaphore:
        ...
        schedule_df, statcast_df = await asyncio.gather(
            _safe_run(executor, fetch_schedule_and_results, team, label=f"[{team.key}] schedule"),
            _safe_run(executor, fetch_statcast_data, team, label=f"[{team.key}] statcast"),
        )
    ...
    try:
        ...
        for team, schedule_df, statcast_df in results:
            ...
            if schedule_df is not None:
                load_to_duckdb(con, schedule_df, "schedule_and_results", append=schedule_loaded)
                schedule_loaded = True
            if statcast_df is not None:
                load_to_duckdb(con, statcast_df, "statcast_pitches", append=statcast_loaded)
